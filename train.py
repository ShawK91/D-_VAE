...
class Parameters:
	def __init__(self):
		...
		self.hidden_size = 150 # fixme: changed from 100 to 150
...
class MERL:
	...
	def train(self, gen, test_tracker):
		...
		if self.args.ps == 'full' or self.args.ps == 'trunk':
			# Homogeneous case is just the popn as a list of lists to maintain compatibility
			teams = [[i] for i in list(range(args.popn_size))] # will just give [[0], [1], [2], ...], as we are using multithreaded network for each pop
		...
		########## START EVO ROLLOUT ##########
		if self.args.popn_size > 0:
			for pipe, team in zip(self.evo_task_pipes, teams): # each team == pop_size will go to the rollout
				pipe[0].send(team)
		...
		if self.args.rollout_size > 0 and not RANDOM_BASELINE:
			...
			#print(self.agents)
			for agent in self.agents: agent.update_rollout_actor() # will update just one network Multithreaded, all agents are in 1 network for homogeneous case
			...
			# Join threads
			for thread in threads: thread.join() # join makes sure that all threads running in parallel have stopped finishing
		...
		if self.args.popn_size > 0:
			...
			# just 1 agent for homogeneous case
				for agent_id, popn_id in enumerate(team): self.agents[agent_id].fitnesses[popn_id].append(
					utils.list_mean(fitness))  ##Assign fitness of team to each of the agents (in homogenenous, there is only 1 agent)
		...
		# Evolution Step
		for agent in self.agents: # depending on the fitness that each agent got, the agents networks will evolve now
			agent.evolve()
